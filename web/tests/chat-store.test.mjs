...
const entry = (id, extra = {}) => ({ messageId: `server-${id}`, clientMessageId: `client-${id}`,
  ...
  sentAt: '2026-01-01T00:00:00.000Z', ...extra });
function store(...limits) { const value = new ChatStore(...limits); value.localId = 'local'; return value; }
...
test('public and private optimistic messages deduplicate acknowledgments, events, and replay', () => {
  ...
  for (const recipientId of [undefined, 'alice']) {
    const message = entry(recipientId ?? 'public', { participantId: 'local', recipientId, recipientName: 'Alice' });
    assert.equal(value.pending({ ...message, messageId: `pending:${message.clientMessageId}` }), true);
  ...
  assert.ok(value.messages.every(message => message.status === 'sent' && message.error === undefined));
...
test('PM privacy and conversation identity are enforced by the session store', () => {
  ...
  assert.equal(value.receive(entry('valid', { recipientId: undefined })), false, 'same server ID cannot change audience');
...
test('replay merges chronologically and unseen entries, not duplicates, contribute unread', () => {
  ...
  value.receive(newer); value.receive(older, true); value.receive(newer, true);
  assert.deepEqual(value.messages.map(message => message.messageId), ['server-older', 'server-newer']);
  assert.equal(value.unread.get('alice'), 2);
  value.open('alice');
  assert.equal(value.unread.size, 0);
  value.receive(older, true);
  assert.equal(value.unread.size, 0);
  value.markUnread(newer); value.markUnread(newer);
  assert.equal(value.unread.get('alice'), 1);
  value.markRead('alice'); assert.equal(value.unread.size, 0);
});
...
test('closing a PM suppresses its late ACK and replay without hiding new messages or trusting clocks', () => {
  const value = store();
  const pending = entry('pending', { participantId: 'local', recipientId: 'alice', sentAt: '2099-01-01T00:00:00Z' });
  value.pending(pending); value.open('alice', 'Alice'); value.close('alice');
  assert.equal(value.receive({ ...pending, sentAt: '2026-01-01T00:00:00Z' }), false);
  assert.equal(value.names.size, 0);
  assert.equal(value.receive(entry('new', { recipientId: 'local', sentAt: '2000-01-01T00:00:00Z' })), true);
  assert.equal(value.names.get('alice'), 'Alice');
  assert.equal(value.unread.get('alice'), 1);
...
test('conversation metadata is bounded even for empty opened and closed conversations', () => {
  ...
  assert.equal(value.names.get('alice'), 'Alice', 'no presence event erases the offline conversation');
  value.reset('other-account');
  assert.equal(value.names.size, 0); assert.equal(value.messages.length, 0); assert.equal(value.unread.size, 0);
});
...
test('drafts and sent-input recall never cross public/private conversation boundaries', () => {
  ...
  inputs.reset(); assert.equal(inputs.draft('public'), '');
});
...
test('input history, draft length, conversation metadata, and total composition memory are bounded', () => {
  ...
  inputs.save('public', 'x'.repeat(3000)); assert.equal(inputs.draft('public').length, 2000);
  for (let id = 0; id < 150; id++) inputs.save(`guest-${id}`, 'private');
  assert.ok(inputs.conversations.size <= 100);
  for (let id = 0; id < 100; id++) for (let message = 0; message < 5; message++) inputs.sent(`guest-${id}`, 'x'.repeat(2000));
  const characters = [...inputs.conversations.values()].reduce((total, value) =>
    total + value.draft.length + value.beforeRecall.length + value.history.join('').length, 0);
