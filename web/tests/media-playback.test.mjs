...
const denied = () => Object.assign(new Error('User activation required'), { name: 'NotAllowedError' });

async function fixture(t) {
  const dom = createDOM();
  // Extend the shared text-only DOM only for the controls used by remote tiles.
  Object.defineProperty(dom.Node.prototype, 'dataset', { get() { return this._dataset ??= {}; } });
  Object.defineProperty(dom.Node.prototype, 'classList', { get() {
    return {
  ...
  dom.Node.prototype.add = function (option) { this.append(option); };
  ...
  dom.Node.prototype.querySelectorAll = function (selector) {
    ...
    return this.children.flatMap(child => [
      ...(child.dataset.control === control ? [child] : []), ...child.querySelectorAll(selector),
    ]);
  };
  const state = { plays: [], roomCalls: [], play: async () => { throw denied(); } };
  const { MediaControls } = await loadTypeScript('src/media-controls.ts', {
    modules: { './media': {}, './media-controls.css': {} },
    globals: {
      document: dom.document,
      localStorage: { getItem: () => null, setItem() {} },
      Option: function (label, value) {
        const option = dom.document.createElement('option'); option.textContent = label; option.value = value; return option;
      },
    },
  });
  ...
  const tile = dom.document.createElement('div'); dom.document.body.append(tile);
  function media(kind = 'audio') {
    ...
    element.paused = true; element.muted = false; element.volume = 1;
    element.pause = () => { element.paused = true; };
    element.play = () => {
      state.plays.push(element);
      return state.play(element).then(() => { element.paused = false; });
    };
    tile.append(element);
    return element;
  }
...
test('blocked remote playback exposes an accessible action and its click retries without changing preferences or publishers', async t => {
  ...
  const toolbar = f.document.createElement('div'); f.document.body.append(toolbar); f.controls.mountToolbar(toolbar);
  const master = toolbar.querySelector('input'); master.value = '50'; master.emit('input');
  f.control('volume').value = '60'; f.control('volume').emit('input');
  await flush();
  ...
  f.state.play = async () => {};
  retry.click();
  assert.equal(f.state.plays.length, plays + 1, 'play must run inside the click, before an asynchronous boundary');
  await flush();
...
for (const preference of ['muted', 'hidden', 'silent']) {
  test(`blocked ${preference} playback does not prompt or override the preference`, async t => {
    const f = await fixture(t);
    if (preference === 'silent') { f.control('volume').value = '0'; f.control('volume').emit('input'); }
...
for (const operation of ['detach', 'reset', 'remove']) {
  test(`${operation} prevents a late rejected play from reviving recovery UI`, async t => {
    ...
    pending.reject(denied()); await flush();
...
for (const replacement of ['stream', 'element']) {
  test(`a late rejection for an old ${replacement} cannot mark its replacement as blocked`, async t => {
    const f = await fixture(t);
    await flush();
    const pending = deferred(); f.state.play = () => pending.promise;
    f.control('retry-playback').click();
    f.state.play = async () => {};
    if (replacement === 'stream') f.audio.srcObject = {};
    else { f.audio.remove(); f.media(); }
...
test('recovery waits for every blocked element on its tile and does not retry another tile', async t => {
  const f = await fixture(t);
  const video = f.media('video'); f.controls.attachTile(f.tile, 'alice', 'Alice');
  const other = f.document.createElement('div'); f.document.body.append(other);
  const otherAudio = f.media(); other.append(otherAudio);
  f.controls.attachTile(other, 'alice', 'Alice');
  await flush();
  const otherPlays = f.state.plays.filter(element => element === otherAudio).length;
  f.state.play = async element => { if (element === video) throw denied(); };
  f.control('retry-playback').click(); await flush();
  assert.equal(f.audio.paused, false);
  assert.equal(f.notice.hidden, false, 'one successful element must not hide another blocked element');
  f.state.play = async () => {};
  f.control('retry-playback').click(); await flush();
  assert.equal(f.notice.hidden, true);
  assert.equal(f.state.plays.filter(element => element === otherAudio).length, otherPlays);
  assert.equal(otherAudio.paused, true, 'another tile belonging to the same participant stays untouched');
...
test('non-policy playback rejection does not masquerade as an autoplay permission prompt', async t => {
  const f = await fixture(t);
  f.state.play = async () => { throw Object.assign(new Error('Replaced stream'), { name: 'AbortError' }); };
  f.controls.attachTile(f.tile, 'alice', 'Alice'); await flush();
  assert.equal(f.notice.hidden, true);
});
