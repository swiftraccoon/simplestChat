...
export function createDOM() {
  ...
  class Node {
    ...
    get isConnected() {
      return this === body || Boolean(this.parentNode?.isConnected);
    }
    get textContent() {
    ...
    set innerHTML(_value) {
      throw new Error('Unexpected HTML rendering');
    }
    append(...nodes) {
    ...
    emit(type, event = {}) {
      for (const entry of [...(this.listeners.get(type) ?? [])]) {
        if (entry.once)
          this.listeners.set(
            type,
            this.listeners.get(type).filter((item) => item !== entry),
          );
    ...
    getBoundingClientRect() {
      return { left: 10, top: 10, right: 200, bottom: 200 };
    }
    querySelectorAll(selector) {
      ...
      return this.children.flatMap((child) => [
        ...(selectors.some(
          (value) =>
            value === child.tagName ||
            value === `#${child.id?.toUpperCase()}` ||
            (value.startsWith('.') &&
              (child.className ?? '').toUpperCase().split(/\s+/).includes(value.slice(1))) ||
            (value === 'DIALOG[OPEN]' && child.tagName === 'DIALOG' && child.open),
        )
          ? [child]
          : []),
  ...
  const document = {
    ...
    querySelector: (selector) =>
      selector === '#login-modal .auth-alt-actions' ? loginActions : body.querySelector(selector),
...
export async function uiFixture(extraGlobals = {}) {
  ...
  dom.document.createElement = (tag) => {
    ...
    if (tag === 'canvas') {
      node.getContext = () =>
        state.contextAvailable ? { drawImage: (...args) => state.draws.push(args) } : null;
      node.toDataURL = (...args) => {
        state.encoded.push(args);
  ...
  const globals = {
    ...
    URL: {
      createObjectURL(file) {
        state.objectUrls.push(file);
        return 'blob:test-upload';
      },
      revokeObjectURL(url) {
  ...
  const ui = await loadTypeScript('src/ui.ts', { modules: await loadContractModules(), globals });
