...
function receiveOnlyGatheringComplete() {
  ...
  return (
    ...
    window.__communityPeerEvents
      ?.get(peer)
      ?.events.some((event) => event.event === 'icecandidate' && event.phase === 'candidate') ===
      true
  );
}
...
async function run() {
  ...
  const artifacts =
    ...
    fs.mkdtempSync(path.join(os.tmpdir(), 'simplestchat-ice-isolation.'));
  fs.mkdirSync(artifacts, { recursive: true, mode: 0o700 });
  const report = {
    browser: options.name,
    browserVersion: null,
    playwrightVersion: require(path.join(path.dirname(require.resolve(moduleName)), 'package.json'))
      .version,
    ...
    capture: false,
    iceServers: [],
    gatheringDeadlineMs: 15000,
    startedAt: new Date().toISOString(),
    complete: false,
    passed: false,
    cases: [],
    limitations: [
      'Gathering only: no ICE-lite server, application negotiation, candidate exchange, connectivity or media delivery.',
    ],
  };
  const save = () =>
    fs.writeFileSync(
      path.join(artifacts, 'ice-isolation-results.json'),
  ...
  for (const name of ['offerer', 'answerer']) {
    ...
    try {
      ...
      await context.route('**/*', (route) =>
        route.request().url() === origin
          ? route.fulfill({
              ...
              body: '<!doctype html><title>ICE isolation</title>',
            })
          : route.abort(),
      );
      ...
      await page.waitForFunction(receiveOnlyGatheringComplete, null, {
        timeout: report.gatheringDeadlineMs,
      });
      result.passed = true;
      ...
      if (page) {
        try {
          result.media = await deadline(page.evaluate(collectPeerDiagnostics), 5000);
        } catch {
          result.diagnosticsError = 'Peer snapshot unavailable';
          result.passed = false;
        }
        try {
          await deadline(
          ...
          result.cleanupError = 'Peer cleanup failed';
          result.passed = false;
        }
      }
      for (const resource of [context, browser]) {
        ...
        try {
          await deadline(resource.close(), 5000);
        } catch {
          result.cleanupError = 'Browser cleanup failed';
          result.passed = false;
        }
      }
      save();
    ...
    console.log(
      `${result.passed ? 'PASS' : 'FAIL'} receive-only ICE ${name}${result.error ? `: ${result.error}` : ''}`,
    );
  }
...
if (require.main === module) {
  if (process.env.ICE_ISOLATION_E2E !== '1') {
    ...
    run().catch(() => {
      console.error(
        'ICE isolation runner failed; check the browser selection, installation and artifact directory.',
      );
      process.exitCode = 1;
    });
  }
