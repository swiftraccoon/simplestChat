...
function note(msg) {
  ...
  results.push({ id: 'note', name: msg, pass: null });
}
...
async function waitConnected(page) {
  await waitFor(
    page,
    () => page.locator('#connection-status.connected').isVisible(),
    'WS connected',
  );
}
...
(async () => {
  ...
  pageA.on('console', (m) =>
    consA.write(`${new Date().toISOString()} [${m.type()}] ${m.text()}\n`),
  );
  pageB.on('console', (m) =>
    consB.write(`${new Date().toISOString()} [${m.type()}] ${m.text()}\n`),
  );
  ...
  await check('pw1', 'Owner joins own password room without prompt', async () => {
    ...
    if (dlgA.messages.filter((m) => m.type === 'prompt').length !== promptsBefore)
      throw new Error('owner was prompted for own room password');
    await pageA.click('#leave-btn');
  ...
  await check('pw2', 'Guest with wrong password is rejected', async () => {
    ...
    await waitFor(
      pageB,
      () =>
        Promise.resolve(dlgB.messages.filter((m) => m.type === 'prompt').length > promptsBefore),
    ...
    await waitFor(
      ...
      () =>
        Promise.resolve(
          dlgB.messages.some((m) => m.type === 'alert' && /password/i.test(m.message)),
        ),
      'wrong-password alert',
      8000,
    );
    if (await vis(pageB, '#room-screen'))
      throw new Error('guest entered password room with wrong password');
  });
  ...
  await check(
    '7b',
    'Admitted user gets media: consumes owner video; produce blocked while unvoiced',
    async () => {
      ...
      await waitFor(
        ...
        'produce-denied toast for unvoiced user',
        8000,
      );
      if ((await pageB.locator('#local-tile video').count()) > 0)
        throw new Error('unvoiced user produced video in moderated room');
    },
  );
  ...
  await check(9, 'Context menu: all 9 actions present for owner', async () => {
    ...
    for (const e of expected)
      if (!direct.includes(e)) throw new Error(`missing action "${e}" (got: ${direct.join(', ')})`);
  ...
  await check('10a', 'Set Role sub-menu shows correct options for owner', async () => {
    ...
    if (JSON.stringify(roleOpts) !== JSON.stringify(want))
      throw new Error(`owner roles: ${roleOpts.join(',')}`);
    await closeModMenu(pageA);
  });
  ...
  await check(11, 'Voice request shows Grant/Dismiss, Grant promotes to member', async () => {
    await waitFor(
      pageB,
      () => vis(pageB, '#hand-btn'),
      'hand button visible for plain user in moderated room',
    );
    ...
    if (!toastTxt.includes(GUEST_NAME) || !toastTxt.includes('requesting voice'))
      throw new Error(`toast: "${toastTxt}"`);
    const btns = await pageA.locator('.toast-action .toast-actions button').allTextContents();
    if (!btns.includes('Grant') || !btns.includes('Dismiss'))
      throw new Error(`buttons: ${btns.join(',')}`);
  ...
  await check('as1', 'Speaking member is highlighted on other clients', async () => {
    ...
    await waitFor(
      ...
      () =>
        pageA
          .locator('.video-tile.speaking, .video-tile.dominant-speaker')
          .count()
          .then((c) => c > 0),
      'speaking/dominant highlight on A',
      15000,
    );
  ...
  await check('10b', 'Moderator sees mod-only actions and User/Member roles only', async () => {
    ...
    if (JSON.stringify(roleOpts) !== JSON.stringify(['User', 'Member']))
      throw new Error(`mod roles: ${roleOpts.join(',')}`);
  ...
  await check(14, 'Topic displays in header, click-to-edit for admins', async () => {
    ...
    if (prompts > 1)
      note(
        `BUG CONFIRMED: topic click fired ${prompts} prompts (listener added per join; main.ts:1033)`,
      );
    await waitFor(
      pageA,
      async () => (await pageA.locator('#room-topic').textContent()) === 'Updated mod topic',
      'topic updated on A',
    );
    await waitFor(
      pageB,
      async () => (await pageB.locator('#room-topic').textContent()) === 'Updated mod topic',
      'topic propagated to B',
    );
    ...
    if (dlgB.messages.filter((m) => m.type === 'prompt').length !== bPrompts)
      throw new Error('moderator got topic edit prompt');
  });
  ...
  await check(15, 'Chat disabled when allowChat=false (and works when on)', async () => {
    ...
    await waitFor(
      pageA,
      async () => (await pageA.locator('#chat-messages').textContent()).includes('hello from B'),
      'chat delivered',
    );
    ...
    await waitFor(
      ...
      () =>
        pageB
          .locator('#chat-input:disabled')
          .count()
          .then((c) => c > 0),
      'B chat input disabled',
    );
    ...
    await waitFor(
      ...
      () =>
        pageB
          .locator('#chat-input:not(:disabled)')
          .count()
          .then((c) => c > 0),
      'B chat re-enabled',
    );
  ...
  await check('8b', 'Lobby deny returns guest to join screen with alert', async () => {
    ...
    await waitFor(
      pageA,
      async () => (await pageA.locator('#lobby-list').textContent()).includes(GUEST_NAME),
      'guest listed',
    );
    const before = dlgB.messages.length;
  ...
  await check('12b', 'Banned user sees alert; rejoin behavior recorded', async () => {
    ...
    await waitFor(
      pageB,
      () => Promise.resolve(dlgB.messages.slice(before2).some((m) => /banned/i.test(m.message))),
      'rejoin-blocked alert',
      8000,
    );
  ...
  fs.writeFileSync(
    `${ART}/results.json`,
    JSON.stringify({ run: TS, results, dialogsA: dlgA.messages, dialogsB: dlgB.messages }, null, 2),
  );
  ...
  fs.writeFileSync(
    `${ART}/results.json`,
    JSON.stringify({ run: TS, results, fatal: String(e) }, null, 2),
  );
  process.exit(2);
});
