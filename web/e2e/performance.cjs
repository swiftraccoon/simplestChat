...
if (process.env.PERFORMANCE_E2E !== '1') throw new Error('Set PERFORMANCE_E2E=1 against an owned disposable local server/database.');
...
const artifacts = process.env.E2E_ARTIFACTS || fs.mkdtempSync(path.join(os.tmpdir(), 'simplestchat-browser-perf.'));
...
const command = (name, args) => { try { return execFileSync(name, args, { encoding: 'utf8' }).trim(); } catch { return null; } };
const report = {
  schemaVersion: 1, completed: false, passed: false,
  startedAt: new Date().toISOString(), runId, label: process.env.RUN_LABEL || '',
  provenance: {
    ...
    cargoLockSha256: hashFile(path.join(process.env.TEST_SERVER_WORKDIR || process.cwd(), 'Cargo.lock')),
    webLockSha256: hashFile(path.join(process.env.TEST_SERVER_WORKDIR || process.cwd(), 'web/package-lock.json')),
    ...
    node: process.version, os: os.type(), osRelease: os.release(), arch: os.arch(),
    cpu: os.cpus()[0]?.model, logicalCpus: os.cpus().length, totalMemoryBytes: os.totalmem(),
    postgres: command('postgres', ['--version']), baseUrl: origin.origin,
    serverWorkdir: process.env.TEST_SERVER_WORKDIR || process.cwd(),
    workload: { users: 2, rooms: 1, publicMessages: 10, messageIntervalMs: 650, fakeCameraSeconds: 5 },
  },
  limitations: [
  ...
  startup: [], api: [], chatDeliveryMs: [], media: null, pageErrors: [],
};
const clients = [];
const pendingBodies = [];
let browser;
...
async function makeClient(label) {
  ...
  await context.addInitScript(() => {
    ...
    window.RTCPeerConnection = class extends Original {
      constructor(...args) { super(...args); window.__perfPeers.push(this); }
    };
    new PerformanceObserver(list => {
      window.__perfLongTasks.push(...list.getEntries().map(item => ({ startTime: item.startTime, duration: item.duration })));
  ...
  const assets = [];
  page.on('pageerror', error => report.pageErrors.push({ client: label, message: error.message }));
  page.on('response', response => {
    if (['script', 'stylesheet'].includes(response.request().resourceType())) {
      pendingBodies.push(response.body().then(body => assets.push({ path: new URL(response.url()).pathname, decodedBytes: body.length, sha256: sha256(body) })));
    }
  });
  page.on('requestfinished', request => {
    const url = new URL(request.url());
    if (url.pathname.startsWith('/api/')) {
      report.api.push({ client: label, method: request.method(), path: url.pathname, timing: request.timing() });
    }
  });
  const item = { page, context, cdp, label }; clients.push(item);
  ...
  const timing = await page.evaluate(() => ({
    ...
    resources: performance.getEntriesByType('resource').map(item => ({
      path: new URL(item.name).pathname, initiatorType: item.initiatorType,
      transferSize: item.transferSize, encodedBodySize: item.encodedBodySize,
      decodedBodySize: item.decodedBodySize, duration: item.duration,
    })),
  ...
  report.startup.push({ client: label, readyWallMs: performance.now() - start, ...timing,
    taskDurationSeconds: metrics.TaskDuration, scriptDurationSeconds: metrics.ScriptDuration,
    jsHeapUsedBytes: metrics.JSHeapUsedSize, jsHeapTotalBytes: metrics.JSHeapTotalSize,
    assets,
  });
  return page;
}
  ...
  return page.evaluate(async () => {
    ...
    for (const peer of window.__perfPeers) {
      for (const stat of (await peer.getStats()).values()) {
        ...
        const fields = ['id', 'kind', 'timestamp', 'bytesReceived', 'packetsReceived', 'packetsLost', 'jitter', 'framesReceived', 'framesDecoded', 'framesDropped', 'frameWidth', 'frameHeight', 'framesPerSecond', 'totalDecodeTime', 'totalFreezesDuration', 'freezeCount'];
        results.push(Object.fromEntries(fields.filter(key => stat[key] !== undefined).map(key => [key, stat[key]])));
      }
    }
    return results;
  });
...
(async () => {
  try {
    ...
    await register(owner, 'owner'); await register(member, 'member');
    ...
    await owner.locator('#create-room-submit').click(); await connected(owner);
    ...
    await member.locator('#join-btn').click(); await connected(member);
    ...
    const setup = owner.getByRole('dialog', { name: 'Camera & microphone', exact: true });
    ...
    await member.waitForFunction(() => [...document.querySelectorAll('.video-tile:not(.local) video')].some(video => video.videoWidth > 0));
    ...
    report.media = { firstDecodedFrameMs, before, after,
    ...
    report.cleanup = await Promise.all(clients.map(async ({ page, label }) => ({
      client: label, ...(await page.evaluate(() => ({
        openPeerConnections: window.__perfPeers.filter(peer => peer.connectionState !== 'closed').length,
        attachedMediaStreams: [...document.querySelectorAll('video,audio')].filter(element => element.srcObject).length,
      }))),
    })));
    ...
    fs.writeFileSync(path.join(artifacts, 'browser-performance.json'), JSON.stringify(report, null, 2));
