...
| `WEBRTC_MIN_OUTGOING_BITRATE` | `100000` | Floor in bit/s for each viewer's send-side bandwidth estimate (0–50000000). `0` keeps mediasoup's own 30 kbit/s floor; a higher floor keeps sending above a weak link's estimate and makes the worker log an error on every bitrate update whose estimate sits below it |
