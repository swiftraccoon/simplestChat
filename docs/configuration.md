...
| `WEBRTC_MIN_OUTGOING_BITRATE` | `100000` | Floor in bit/s for each viewer's send-side bandwidth estimate (0–50000000). `0` keeps mediasoup's own 30 kbit/s floor; the default keeps the lowest simulcast layer flowing at a 150 kbit/s cap where mediasoup's floor loses frames (see the performance results of 2026-09-21) |
...
| `CPU_SATURATION_WORKER_UTILIZATION` | `0.85` | Share of one core a media worker thread may use over the last 10 s before it counts as saturated (0.2–1); clears below 80 % of this. A mediasoup router lives on one worker thread, so new rooms avoid a saturated worker, rooms already on it refuse fresh joins, and `/ready` fails only when every worker is saturated. Linux only (reads `/proc/self/task/<tid>/schedstat`); elsewhere the per-worker gauges are absent |
