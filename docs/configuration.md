...
| `MEDIA_WORKER_LOG_LEVEL` | `warn` | What the native media worker logs: `debug`, `warn`, `error` or `none`; `debug` with `MEDIA_WORKER_LOG_TAGS=ice,dtls` records each transport's ICE and DTLS state machine for a diagnosis run (also raise the process filter, `RUST_LOG=simplestChat=info,mediasoup=debug`) |
| `MEDIA_WORKER_LOG_TAGS` | `info,ice,dtls,rtp,rtcp` | Comma-separated worker log tags: `info`, `ice`, `dtls`, `rtp`, `srtp`, `rtcp`, `rtx`, `bwe`, `score`, `simulcast`, `svc`, `sctp`, `message`; an unknown tag fails startup |
...
| `WEBRTC_RECV_BUFFER_BYTES` | `1048576` | Receive buffer each media worker requests for its WebRTC listener (`0` keeps the kernel default, otherwise 65536–67108864 bytes). The Linux default of 208 KiB holds about 14 ms of the inbound media of 200 publishers; at that size the kernel sheds millisecond bursts throughout a loaded room (1–4 % of the primary worker's inbound datagrams on the two-worker production shape), and those bursts are what cost handshakes their final flight. 1 MiB absorbs them (no drop in three of four runs, no client lost outside sustained overload). It is not a lever against a worker that falls behind for seconds: that overload drops a quarter of the inbound at any size, so size rooms with `MAX_PARTICIPANTS_PER_ROOM`, and a much deeper buffer only adds queueing delay (4 MiB: up to 1.2 s; see [performance results](performance-results.md)). The kernel clamps the request to `net.core.rmem_max` without saying so, so the host must raise that sysctl for the size to apply ([deployment](deployment.md)); the server logs a warning at startup when the ceiling is below the request |
| `WEBRTC_SEND_BUFFER_BYTES` | `1048576` | Send buffer each media worker requests for its WebRTC listener, same range and rules as the receive buffer, clamped to `net.core.wmem_max` |
...
| `CPU_SATURATION_WORKER_UTILIZATION` | `0.7` | Share of one core a media worker thread may use over the last 10 s before it counts as saturated (0.2–1); clears below 80 % of this. This is the leading admission signal: at 0.85 a saturated one-to-many ramp killed the process before any guard fired, at 0.7 it refused joins at about 730 viewers with the room intact (see [performance results](performance-results.md)). A room's router lives on one worker thread, so new rooms avoid a saturated worker, rooms already on it refuse fresh joins, and `/ready` fails only when every worker is saturated. Once a room's worker carries 64 or more consumers, new receive transports are placed on the least loaded worker and the room's producers are piped to a viewer router there, so a large one-to-many room spreads its viewers across workers while its producers stay on the primary worker. Linux only (reads `/proc/self/task/<tid>/schedstat`); elsewhere the per-worker gauges are absent |
| `MEMORY_SATURATION_FRACTION` | `0.85` | Share of the cgroup memory limit in use at which the process counts as saturated (0.5–1); clears below 90 % of this. It stops admissions during slow growth (many rooms, a leak). It does not catch a saturated ramp: on the two-worker production shape a 1,250-viewer ramp went from 73 % of 2 GiB to the OOM kill in about ten seconds and the process stalled before the two-second sampler could react, so keep the worker CPU guard and the room limits as the first line. Inactive without a finite cgroup v2 memory limit |
