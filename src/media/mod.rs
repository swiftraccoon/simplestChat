...
pub use types::{ConsumerInfo, MediaError, MediaResult, ProducerInfo, TransportInfo};
...
impl MediaServer {
    ...
    pub async fn new(config: MediaConfig) -> Result<Self> {
        info!(
            "Initializing MediaServer with {} workers",
    ...
    pub async fn create_router(&self, room_id: String) -> Result<String> {
        ...
        self.router_manager
            .create_router(room_id, self.config.router_config.clone())
            .await
